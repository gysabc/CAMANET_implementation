...
class BaseTrainer(object):
    def __init__(self, model, criterion, metric_ftns, optimizer, lr_scheduler, args, logger, config):
        ...
        if args.addcls: # 区别
            self.cls_criterion = torch.nn.BCEWithLogitsLoss() # 二元交叉熵损失
            self.cls_w = args.cls_w # 权重
        self.attn_cam = args.attn_cam
        self.wmse = args.wmse
        if args.attn_cam: # 区别
            self.mse_criterion = Weighted_MSELoss(weight=args.wmse) # 标准的均方误差损失
            self.mse_w = args.mse_w # 权重
        self.criterion = criterion
        self.metric_ftns = metric_ftns

        self.logger = logger

        self.clip_w = args.clip_w # 另一个损失的权重
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.mnt_mode != 'off':
                try:
                    ...
                    cur_metric = log['val_BLEU_4'] + 0.5 * log['val_BLEU_1']
    ...
    def _record_best(self, log):
        ...
        cur_metric = log['test_BLEU_4'] +  0.5 * log['test_BLEU_1']
        ...
        with tqdm(desc='Epoch %d - train, lr:(%.5f,%.5f)' % (epoch, cur_lr[0], cur_lr[1]),
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks, labels) in enumerate(self.train_dataloader):
                ...
                if logits is not None:
                    # 标签损失(图像分类损失),会累加到总的损失上去
                    img_cls_loss = self.cls_criterion(logits, labels)
                    loss = loss + self.cls_w * img_cls_loss # 权重为1
                ...
                if total_attn is not None:
                    ...
                    loss = loss + self.mse_w * mse_loss # 权重为0.15
                ...
                loss.backward()
                torch.nn.utils.clip_grad_value_(self.model.parameters(), self.clip_value) # 梯度裁剪
        ...
        with tqdm(desc='Epoch %d - val' % epoch, unit='it', total=len(self.val_dataloader)) as pbar:
            with torch.no_grad():
                ...
                for batch_idx, (images_id, images, reports_ids, reports_masks,labels) in enumerate(self.val_dataloader):
                    ...
                    output, _, _ = self.model(images, labels=labels, mode='sample') # sample_beam函数与我的有所不同,但是这里只接收了output,所以就没差别了


                    if total_attn is not None:
                        # 默认执行,即默认在验证集上计算均方误差损失
                        mse_loss = self.mse_criterion(total_attn, fore_map, logits, labels)
